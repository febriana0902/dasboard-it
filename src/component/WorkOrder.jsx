...
const WorkOrder = ({ toggleSidebar, isSidebarOpen }) => {
    ...
    // Filter untuk bar chart hanya berdasarkan selectedTag
    const filteredBarChartData = posts.filter(item => {
        return selectedTag === "" || item.tags.includes(selectedTag);
    });

    // Filter untuk tabel berdasarkan selectedTag dan searchTerm
    const filteredTableData = posts.filter(item => {
        return (
            (selectedTag === "" || item.tags.includes(selectedTag)) &&
            (
                item.title.toLowerCase().includes(searchTerm.toLowerCase()) ||
    ...
    useEffect(() => {
        if (filteredTableData.length === 0) {
            ...
            const maxPage = Math.ceil(filteredTableData.length / entriesPerPage);
            if (currentPage > maxPage) {
                setCurrentPage(maxPage);
            }
        }
    }, [searchTerm, filteredTableData, entriesPerPage, currentPage]);

    // Pagination calculations
    const startIndex = (currentPage - 1) * entriesPerPage;
    const endIndex = startIndex + entriesPerPage;
    const displayedData = filteredTableData.slice(startIndex, endIndex);
    ...
    return (
        <div className="wko-content">
            ...
            <div className='w-background-bar'>
                {/* Bar chart hanya mengikuti filter tag */}
                <BarChart posts={filteredBarChartData} />
                <div className="w-table-container">
                    <div className='w-group-search'>
                        <div className="w-dropdown-container">
                            <label htmlFor="entries">Show </label>
                    ...
                    <div className="w-info-container">
                        <p>Showing {startIndex + 1} to {Math.min(endIndex, filteredTableData.length)} of {filteredTableData.length} entries</p>
                        <div className="w-pagination-container">
                            <FontAwesomeIcon icon={faAnglesLeft} onClick={handlePreviousPage} />
                            <span> {currentPage} </span>
                            <FontAwesomeIcon icon={faAnglesRight} onClick={handleNextPage} />
